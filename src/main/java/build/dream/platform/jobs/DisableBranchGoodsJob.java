...
public class DisableBranchGoodsJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        try {
            boolean setnxSuccessful = CacheUtils.setnx(Constants.KEY_DISABLE_BRANCH_GOODS_JOB_EXECUTE_SIGN, UUID.randomUUID().toString());
            branchService.disableGoods();
            if (setnxSuccessful) {
                CacheUtils.expire(Constants.KEY_DISABLE_BRANCH_GOODS_JOB_EXECUTE_SIGN, 30, TimeUnit.HOURS);
            }
        } catch (Exception e) {
